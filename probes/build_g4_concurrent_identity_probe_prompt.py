...
Qualification requires actual child execution overlap: B must start before A reaches accepted terminal completion, and both identities must later complete independently. A rejected SubagentStop correction attempt is not terminal completion. Same-assistant-response batching is not required. If either custom-agent spawn explicitly fails, or if Hook capture, identity binding, wait, callback, final attestation, or actual overlap fails, report the exact non-secret failure and stop. Never reuse either child's task name, thread id, assignment id, handoff id, capsule hash, compact hash, or final result for the other child. Do not fall back to default, explorer, worker, or v4.
