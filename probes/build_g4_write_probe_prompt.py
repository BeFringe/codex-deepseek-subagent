...
def build(
    ...
    *,
    ...
    feasibility_attestation: Mapping[str, object] | None = None,
) -> str:
    root = root.resolve()
    ...
    feasibility = (
        ...
        if feasibility_attestation is None
        else dict(feasibility_attestation)
    )
    bounded = feasibility.get("bounded_completion")
    if not (
        feasibility.get("parent_owner_id") == "phase1.parent"
        and feasibility.get("owner_decision") == "dispatch"
        and isinstance(bounded, Mapping)
        and bounded.get("completion_condition") == completion_condition
        and bounded.get("proposed_mechanism")
        in {
            proposed_mechanism,
            "one concrete Add operation, exact Hook ceiling and writer lease",
        }
        and bounded.get("work_budget")
        == {"unit": "tool-call", "cardinality_domain": "owned-path", "limit": 1}
    ):
        raise PromptError("feasibility attestation does not bind the exact write contract")
    authority = {
        "schema": 1,
        ...
        "execution_contract": {
            ...
            "closed_registries": [],
            "relation_contracts": [],
            "capsule_feasibility_attestation": feasibility,
        },
...
def main() -> int:
    ...
    try:
        ...
        if arguments.feasibility_receipt is not None:
            receipt = json.loads(arguments.feasibility_receipt.read_text(encoding="utf-8"))
            feasibility = receipt.get("attestation") if isinstance(receipt, dict) else None
            if not isinstance(feasibility, dict):
                raise PromptError("feasibility receipt has no attestation")
        prompt = build(
            arguments.root,
            arguments.task_name,
            arguments.target,
    ...
    except (OSError, json.JSONDecodeError, PromptError) as error:
