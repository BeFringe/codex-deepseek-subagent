...
from p7_write_feasibility import collect as collect_feasibility, replace_prompt
...
def main():
    ...
    parser.add_argument('--measured-feasibility', action='store_true')
    parser.add_argument('--parent-claim-barrier', action='store_true')
    args = parser.parse_args()
    require(not (args.measured_feasibility or args.parent_claim_barrier) or args.case == 'positive',
            'measured writer qualification flags require positive case')
    ...
    if args.case == 'positive':
        ...
        if args.measured_feasibility:
            feasibility = collect_feasibility(root, baseline)
            write_json(artifacts / 'feasibility.json', feasibility)
            prompt = replace_prompt(prompt, feasibility)
        if args.parent_claim_barrier:
            prompt = prompt.replace('Do not edit files yourself, change providers, inspect credentials, or use another agent type.',
                                    'Do not change providers, inspect credentials, or use another agent type. The only parent mutation request is the explicit denied qualification below.')
            prompt = prompt.replace('Then use only native wait/callback until that child returns and report its exact result.',
                                    'First perform the parent conflict request specified below, then wait for the child and report its exact result.')
            parent_gate = ('Immediately after spawn returns, call native apply_patch exactly once with the exact patch below. '
                'The isolated Hook will wait for the child writer claim and require a durable rejection before allowing the child to continue. '
                'Do not wait for child completion before this parent call; do not retry the denied request. '
                'Then continue the prescribed wait, callback, close and list sequence.\n\n'
                f'*** Begin Patch\n*** Add File: {root / "qualified.txt"}\n+G4_PARENT_CONFLICT_MUST_NOT_WRITE\n*** End Patch\n\n')
            prompt = prompt.replace('EXACT CHILD ASSIGNMENT:\n', parent_gate + 'EXACT CHILD ASSIGNMENT:\n', 1)
    if args.case == 'negative':
    ...
    if args.case != 'capability':
        final_shape = ('Emit exactly one plain-text final attestation envelope without Markdown or surrounding prose. '
                       ...
                       'The exact top-level JSON keys are assignment_id, handoff_id, capsule_sha256, '
                       'compact_invariant_sha256, canonical_agent_path, recovery_count, root, branch, head, '
                       'index_changed, git_status_short, changed_paths, authority_provenance, verification, '
                       'inventory_summaries, context_lost, authority_violation, assigned_slice_complete. '
                       'authority_provenance is a nested object with exactly policy_sha256, worker_claimed_origin, '
                       'test_only_injection_used, derivation_receipt_sha256. Put test_only_injection_used=false '
                       'inside that object only, never at top level. Do not copy the entire seed as final JSON.\n\n')
    if args.case == 'negative':
    ...
    if args.parent_claim_barrier:
        hook_args += ['--parent-claim-barrier']
    ...
    for groups in hooks['hooks'].values():
        for group in groups:
            for hook in group['hooks']:
                ...
                hook['timeout'] = 150 if args.parent_claim_barrier else 30
    ...
    manifest = {
        ...
        'measured_feasibility': args.measured_feasibility, 'parent_claim_barrier': args.parent_claim_barrier,
        ...
        'harness_sha256': {name: digest(ROOT / 'probes' / name) for name in
            ('run_p7_windows_mutation.py', 'p7_windows_mutation_hook.py', 'p7_windows_acl.py',
             ...
             'p7_write_feasibility.py',
             'adjudicate_p7_windows_writer.py',
    ...
    if args.measured_feasibility:
        manifest['artifacts']['feasibility.json'] = digest(artifacts / 'feasibility.json')
