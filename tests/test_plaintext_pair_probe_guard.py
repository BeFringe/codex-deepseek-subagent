...
class PlaintextPairProbeGuardTests(unittest.TestCase):
    ...
    def test_state_directory_must_be_canonical_temporary_root_descendant(self):
        temporary_root = Path(
            tempfile.gettempdir() if sys.platform == "win32" else "/private/tmp"
        ).resolve(strict=True)
        outside = Path(temporary_root.anchor) / "codex-p1-pair-guard-outside"
        with self.assertRaisesRegex(guard.ProbeGuardError, "temporary-root"):
            guard.validate_state_directory(outside)
