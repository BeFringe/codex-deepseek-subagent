...
class G4WriteProbePromptTests(unittest.TestCase):
    def setUp(self):
        ...
        self.feasibility = Path(self.directory.name + ".feasibility.json")

    def tearDown(self):
        if self.output.exists():
            self.output.unlink()
        if self.feasibility.exists():
            self.feasibility.unlink()
    ...
    def run_builder(
        ...
        *,
        ...
        if feasibility_receipt is not None:
            command += ["--feasibility-receipt", str(feasibility_receipt)]
    ...
    def test_prompt_embeds_exact_parent_owned_feasibility_receipt(self):
        attestation = {
            "parent_owner_id": "phase1.parent",
            "exact_claimed_invariant": "one exact owned-path apply_patch closes within one tool call",
            "counterexample_probe": {
                "probe_id": "live-foreign-drift-negative",
        ...
        self.feasibility.write_text(
            json.dumps({"attestation": attestation}) + "\n", encoding="utf-8"
        )
        result = self.run_builder(feasibility_receipt=self.feasibility)
        self.assertEqual(result.returncode, 0, result.stderr)
        prompt = self.output.read_text(encoding="utf-8")
        declaration = json.loads(
            prompt.split("BEGIN CODEX WORKER AUTHORITY\n", 1)[1].split(
        ...
        self.assertNotIn('"1111111111111111', prompt)

    def test_feasibility_receipt_must_bind_exact_contract(self):
        self.feasibility.write_text(
            json.dumps(
                {
                    "attestation": {
                        ...
                        "owner_decision": "dispatch",
                        "bounded_completion": {
                            "completion_condition": "different work",
                            "proposed_mechanism": "exact Hook ceiling plus writer lease",
                            "work_budget": {
                                "unit": "tool-call",
                                "cardinality_domain": "owned-path",
