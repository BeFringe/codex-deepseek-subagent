...
class G4SessionCloseProbePromptTests(unittest.TestCase):
    ...
    def test_prompt_orders_live_tree_close_and_post_close_observation(self):
        prompt = self.build()
        spawn = prompt.index("g4_assignment.spawn_agent tool exactly once")
        first_listing = prompt.index("g4_assignment.list_agents once")
        close = prompt.index("g4_assignment.close_agent tool exactly once")
        second_listing = prompt.index("g4_assignment.list_agents once more")

        self.assertLess(spawn, first_listing)
        self.assertLess(first_listing, close)
        self.assertLess(close, second_listing)
        self.assertIn("Without any wait_agent call", prompt)
        self.assertIn("require the exact child to be running", prompt)
        self.assertNotIn("timeout_ms=", prompt)
        ...
        self.assertIn("unconfirmed_exit_process_ids_by_thread", prompt)
        self.assertIn("unresolved_start_process_ids_by_thread", prompt)
        ...
        self.assertIn("not filesystem, later-spawned, detached, untracked", prompt)
    ...
    def test_capsule_is_short_lived_read_only_and_exactly_bound(self):
        ...
        self.assertIn("immediately closes", declaration["stop_condition"])
